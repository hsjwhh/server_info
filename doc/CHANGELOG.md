...
### ✨ 新特性
- **CPU 多维组合搜索**：在配置方案页引入“核心数”筛选维度，支持“型号 + 核心数”的复合过滤，并重构了后端请求参数结构以支持灵活扩展。
- **业务筛选**：在服务器搜索结果页新增“业务筛选”下拉框
，支持基于搜索结果的内存过滤、分页联动以及 Session 状态持久化。
...
### 🚀 优化与重构
- **图标加载稳定性增强**：撤销了不稳定或无法下载的 `@mdi/font` CDN 引用，恢复为本地 npm 包引入模式，并同步优化了 Vite 的 `manualChunks` 策略（隔离至 `icons-vendor`），确保在断网或内网环境下图标 100% 可用。
- **视觉细节微调**：
...
### 🐛 修复
- **API 兼容性**：同步修复了服务器入库页面的 CPU 搜索逻辑，确保其适配重构后的对象传参格式。
- **全量数据排序**：
重构了服务器列表的排序逻辑，将排序行为从组件局部移至 Store 层级（`sortedServers`），实现了基于全量搜索结果的跨页排序，并解决了日期（y/m/d 分散字段）排序乱序的问题。
