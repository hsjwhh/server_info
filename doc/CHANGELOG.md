...
### 🚀 优化与重构
- **全站页面瘦身**：对 `pages/` 目录下所有页面进行了样式治理，移除了约 100+ 行重复的 Scoped CSS，全面改用全局 `utilities` 与 `Design Token`。
- **布局一致性修复**：移除了所有路由页面根部的 Flex 容器，回归块级流式布局，解决了内容不足时页面“垂直居中”导致的顶部空白问题。
- **样式体系重构**：建立了完整的 Design Token 体系 (`tokens.css`)，消灭硬编码颜色，统一了全站间距、圆角与阴影。
- **动效修复**：重构了 Sidebar 折叠动效，解决了过渡动画被阻断的问题。
